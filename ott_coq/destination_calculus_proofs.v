...
Lemma ValidOnlyHvarShiftEquiv: forall (G : ctx) (H : hvars) (h' : hvar), ValidOnly G <-> ValidOnly (G ᴳ[ H⩲h' ]).
Proof.
  intros *. unfold ValidOnly, ctx_hvar_shift.
  rewrite map_propagate_both with (Q := fun x b => IsValid (binding_mode b)).
...
Lemma DestOnlyHvarShiftEquiv: forall (G : ctx) (H : hvars) (h' : hvar), DestOnly G <-> DestOnly (G ᴳ[ H⩲h' ]).
Proof.
  intros *. unfold DestOnly, ctx_hvar_shift.
...
Lemma LinNuOnlyHvarShiftEquiv : forall (G : ctx) (H : hvars) (h' : hvar), LinNuOnly G <-> LinNuOnly (G ᴳ[ H⩲h' ]).
Proof.
  intros *. unfold LinNuOnly, ctx_hvar_shift.
  rewrite map_propagate_both with (Q := fun x b => IsLinNu (binding_mode b)).
  2:{ intros [xx|xh] **. all: cbn.
      all: reflexivity. }
  apply precomp_propagate_both. intros x2.
  sfirstorder use: preshift_surjective.
...
Lemma LinOnlyHvarShiftEquiv : forall (G : ctx) (H : hvars) (h' : hvar), LinOnly G <-> LinOnly (G ᴳ[ H⩲h' ]).
Proof.
  intros *. unfold LinOnly, ctx_hvar_shift.
  rewrite map_propagate_both with (Q := fun x b => IsLin (binding_mode b)).
  2:{ intros [xx|xh] **. all: cbn.
      all: reflexivity. }
  apply precomp_propagate_both. intros x2.
  sfirstorder use: preshift_surjective.
...
Lemma FinAgeOnlyHvarShiftEquiv : forall (G : ctx) (H : hvars) (h' : hvar), FinAgeOnly G <-> FinAgeOnly (G ᴳ[ H⩲h' ]).
Proof.
  intros *. unfold FinAgeOnly, ctx_hvar_shift.
  rewrite map_propagate_both with (Q := fun x b => IsFinAge (binding_mode b)).
  2:{ intros [xx|xh] **. all: cbn.
      all: reflexivity. }
  apply precomp_propagate_both. intros x2.
  sfirstorder use: preshift_surjective.
...
Lemma hvars_max_HDisjoint : forall (H H' : hvars) (h' : hvar), maxᴴ(H) < h' -> H ## (H' ᴴ⩲ h').
Proof. Admitted.
...
Lemma ValidOnlyUnionBackward : forall (G1 G2 : ctx), ValidOnly (G1 ᴳ+ G2) -> ValidOnly G1 /\ ValidOnly G2.
Proof.
  assert (forall m1 m2, IsValid (mode_plus m1 m2) -> IsValid m1 /\ IsValid m2) as h_mode_plus.
  { intros [[p1 a1]|] [[p2 a2]|]. all: cbn.
    all: sfirstorder. }
  unfold ValidOnly, ctx_union in *. intros *.
...
Lemma ValidOnlyUnionBackward' : forall (G1 G2 : ctx), Basics.impl (ValidOnly (G1 ᴳ+ G2)) (ValidOnly G1 /\ ValidOnly G2).
Proof.
  exact ValidOnlyUnionBackward.
Qed.
Hint Rewrite ValidOnlyUnionBackward' : propagate_down.

Lemma ValidOnlyUnionForward : forall (G1 G2 : ctx), ValidOnly G1 -> ValidOnly G2 -> G1 # G2 -> ValidOnly (G1 ᴳ+ G2).
Proof.
...
Lemma ValidOnlyUnionForward' : forall (G1 G2 : ctx), Basics.impl (ValidOnly G1 /\ ValidOnly G2 /\ G1 # G2) (ValidOnly (G1 ᴳ+ G2)).
Proof.
...
Lemma ValidOnlySingletonVar : forall x m T, ValidOnly ᴳ{ x : m ‗ T} <-> IsValid m.
Proof.
...
Lemma ValidOnlyStimesBackward : forall (m : mode) (G : ctx), ValidOnly (m ᴳ· G) -> ValidOnly G.
Proof.
  intros *.
  intros validmG.
...
Lemma ValidOnlyStimesBackward' : forall (m : mode) (G : ctx), Basics.impl (ValidOnly (m ᴳ· G)) (ValidOnly G).
Proof.
  exact ValidOnlyStimesBackward.
Qed.
Hint Rewrite ValidOnlyStimesBackward' : propagate_down.

Lemma TimesIsValidEquiv : forall (m1 m2 : mode), IsValid (m1 · m2) <-> IsValid m1 /\ IsValid m2.
Proof.
  intros [[p1 a1]|] [[p2 a2]|]. all: cbn.
  all: sfirstorder.
Qed.
...
Lemma ValidOnlyStimesForward : forall (m : mode) (G : ctx), ValidOnly G /\ IsValid m -> ValidOnly (m ᴳ· G).
Proof.
  intros * [validG validm]. unfold ValidOnly, ctx_stimes in *.
...
Lemma ValidOnlyStimesForward' : forall (m : mode) (G : ctx), Basics.impl (ValidOnly G /\ IsValid m) (ValidOnly (m ᴳ· G)).
Proof.
  intros *. unfold Basics.impl.
...
Lemma DestOnlyUnionEquiv : forall (G1 G2 : ctx), DestOnly (G1 ᴳ+ G2) <-> DestOnly G1 /\ DestOnly G2.
Proof.
  intros *. unfold DestOnly, ctx_union.
...
Lemma DestOnlyStimesEquiv : forall (m : mode) (G : ctx), DestOnly G <-> DestOnly (m ᴳ· G).
Proof.
  intros *. unfold DestOnly, ctx_stimes.
...
Lemma mode_plus_not_lin : forall b1 b2, ~IsLin (mode_plus b1 b2).
Proof.
  intros [[q1 [a1|]]|].
  2:{ cbn.  sauto q: on. }
  2:{ cbn. sauto lq: on. }
...
Lemma IsLinNuWkIsLin : forall (m : mode), IsLinNu m -> IsLin m.
Proof.
  intros *.
  sauto lq: on.
Qed.

Lemma IsLinNuWkIsLin' : forall (m : mode), Basics.impl (IsLinNu m) (IsLinNu m /\ IsLin m).
Proof.
  sfirstorder use: IsLinNuWkIsLin.
Qed.
Hint Rewrite IsLinNuWkIsLin' : saturate.

Lemma IsLinWkIsValid : forall (m : mode), IsLin m -> IsValid m.
Proof.
  intros m H. destruct H. apply (IsValidProof (Lin, a)).
Qed.

Lemma IsLinWkIsValid' : forall (m : mode), Basics.impl (IsLin m) (IsLin m /\ IsValid m).
Proof.
  sfirstorder use: IsLinWkIsValid.
Qed.
Hint Rewrite IsLinWkIsValid' : saturate.

Lemma mode_plus_not_lin_nu : forall b1 b2, ~IsLinNu (mode_plus b1 b2).
Proof.
  intros b1 b2 h.
...
Lemma LinOnlyUnionEquiv : forall (G1 G2 : ctx), LinOnly (G1 ᴳ+ G2) <-> LinOnly G1 /\ LinOnly G2 /\ G1 # G2.
Proof.
  intros *.
...
Lemma LinNuOnlyWkLinOnly : forall (G : ctx), LinNuOnly G -> LinOnly G.
Proof.
  intros *.
  sfirstorder use: IsLinNuWkIsLin.
Qed.

Lemma LinOnlyWkValidOnly : forall (G : ctx), LinOnly G -> ValidOnly G.
Proof.
  intros *.
  sfirstorder use: IsLinWkIsValid.
Qed.

Lemma LinNuOnlyUnionEquiv : forall (G1 G2 : ctx), LinNuOnly (G1 ᴳ+ G2) <-> LinNuOnly G1 /\ LinNuOnly G2 /\ G1 # G2.
Proof.
  intros *.
  split.
  - intros h.
    ...
    2:{ hauto lq: on. }
    unfold LinNuOnly, ctx_union in *.
    eapply merge_with_propagate_backward_disjoint'.
    { apply disj. }
    eauto.
  - intros h. unfold LinNuOnly, ctx_union in *.
    apply merge_with_propagate_forward_disjoint.
    all: sfirstorder.
Qed.
Hint Rewrite LinNuOnlyUnionEquiv : propagate_down.

Lemma LinNuOnlyStimesForward : forall (m : mode) (G : ctx), IsLinNu m -> LinNuOnly G -> LinNuOnly (m ᴳ· G).
Proof.
  intros * linm linG.
  unfold LinNuOnly in *.
  intros n b h.
  ...
  - unfold binding_stimes_var in H0. destruct x. subst. unfold binding_mode in *. unfold IsLinNu in *. subst. unfold mode_times. cbn. reflexivity.
  - unfold binding_stimes_dh in H0. destruct x; subst.
    + unfold binding_mode in *. unfold IsLinNu in *. subst. unfold mode_times. cbn. reflexivity.
    + unfold binding_mode in *. unfold IsLinNu in *. subst. unfold mode_times. cbn. reflexivity.
Qed.
Lemma LinNuOnlyStimesForward' : forall (m : mode) (G : ctx), Basics.impl (IsLinNu m /\ LinNuOnly G) (LinNuOnly (m ᴳ· G)).
Proof.
  intros *. unfold Basics.impl.
...
Lemma LinNuOnlyStimesBackward : forall (m : mode) (G : ctx), LinNuOnly (m ᴳ· G) -> LinNuOnly G.
Proof.
  intros *.
  intros islinNu.
  pose (fun n : name => match n as n0 return (binding_type_of n0 -> binding_type_of n0) with
  ...
  - destruct binding. unfold binding_stimes_var in *. unfold mode_times, IsLinNu in *. destruct m, m0; cbn; try destruct p; try destruct p0.
...
Proof.
  ...
  - intros [m1 ? ?|? m1] [m2 ? ?|? m2]. all: cbn.
    ...
    all: sfirstorder.
Qed.
Lemma FinAgeOnlyUnionBackward' : forall (G1 G2 : ctx), Basics.impl (FinAgeOnly (G1 ᴳ+ G2)) (FinAgeOnly G1 /\ FinAgeOnly G2).
Proof.
  exact FinAgeOnlyUnionBackward.
Qed.
Hint Rewrite FinAgeOnlyUnionBackward' : propagate_down.

Lemma FinAgeOnlyUnionForward : forall (G1 G2 : ctx), (FinAgeOnly G1 /\ FinAgeOnly G2 /\ G1 # G2) -> FinAgeOnly (G1 ᴳ+ G2).
Proof.
  intros * h. unfold ctx_union, FinAgeOnly.
  apply merge_with_propagate_forward_disjoint.
  all: sfirstorder.
Qed.
Lemma FinAgeOnlyUnionForward' : forall (G1 G2 : ctx), Basics.impl (FinAgeOnly G1 /\ FinAgeOnly G2 /\ G1 # G2) (FinAgeOnly (G1 ᴳ+ G2)).
Proof.
  intros *. unfold Basics.impl.
...
Lemma LinOnlyStimesBackward : forall (m : mode) (G : ctx), LinOnly (m ᴳ· G) -> LinOnly G.
Proof.
  intros *.
  intros islin.
  pose (fun n : name => match n as n0 return (binding_type_of n0 -> binding_type_of n0) with
...
Qed.
Lemma LinOnlyStimesBackward' : forall (m : mode) (G : ctx), Basics.impl (LinOnly (m ᴳ· G)) (LinOnly G).
Proof.
  exact LinOnlyStimesBackward.
Qed.
Hint Rewrite LinOnlyStimesBackward' : propagate_down.

Lemma LinOnlyStimesForward : forall (m : mode) (G : ctx), IsLin m -> LinOnly G -> LinOnly (m ᴳ· G).
Proof.
  intros * validm linG.
  unfold LinOnly in *.
  intros n b h.
...
Qed.
Lemma LinOnlyStimesForward' : forall (m : mode) (G : ctx), Basics.impl (IsLin m /\ LinOnly G) (LinOnly (m ᴳ· G)).
Proof.
  intros *. unfold Basics.impl.
...
Lemma FinAgeOnlyStimesBackward : forall (m : mode) (G : ctx), FinAgeOnly (m ᴳ· G) -> FinAgeOnly G.
Proof.
  intros *.
  intros isfinAge.
  pose (fun n : name => match n as n0 return (binding_type_of n0 -> binding_type_of n0) with
...
Qed.
Lemma FinAgeOnlyStimesBackward' : forall (m : mode) (G : ctx), Basics.impl (FinAgeOnly (m ᴳ· G)) (FinAgeOnly G).
Proof.
  exact FinAgeOnlyStimesBackward.
Qed.
Hint Rewrite FinAgeOnlyStimesBackward' : propagate_down.

Lemma FinAgeOnlyStimesForward : forall (m : mode) (G : ctx), IsFinAge m -> FinAgeOnly G -> FinAgeOnly (m ᴳ· G).
Proof.
...
Qed.
Lemma FinAgeOnlyStimesForward' : forall (m : mode) (G : ctx), Basics.impl (IsFinAge m /\ FinAgeOnly G) (FinAgeOnly (m ᴳ· G)).
Proof.
  intros *. unfold Basics.impl.
...
Proof.
  (* This proof, and the similar ones below are more complicated than
    ...
    copy-paste will do. *)
  intros *. unfold Disjoint, ctx_stimes.
  split.
  - intros h x.
    specialize (h x).
...
Proof.
  intros *. unfold Disjoint, ctx_minus.
  split.
  - intros h x.
    rewrite In_map_iff.
    eauto.
...
Proof.
  intros *. unfold Disjoint, ctx_invminus.
  split.
  - intros h x.
    rewrite In_map_iff.
    eauto.
...
Proof.
  intros *. unfold Disjoint, ctx_union.
  split.
  - intros h.
    split.
    all: intros x.
...
Lemma EmptyIsLinOnly : LinOnly ᴳ{}.
Proof.
  scongruence unfold: LinOnly.
Qed.

Lemma EmptyIsFinAgeOnly : FinAgeOnly ᴳ{}.
Proof.
  scongruence unfold: FinAgeOnly.
Qed.

Lemma EmptyIsDestOnly : DestOnly ᴳ{}.
Proof.
...
Lemma EmptyIsDisjointRight : forall (G : ctx), Disjoint G ᴳ{}.
Proof.
  sauto q: on unfold: Disjoint.
Qed.

Lemma EmptyIsDisposableOnly : DisposableOnly ᴳ{}.
Proof.
...
Lemma DisjointDestOnlyVar : forall (G : ctx) (x : var) (m : mode) (T : type), DestOnly G -> G # (ᴳ{ x : m ‗ T}).
Proof.
  intros * destonly.
...
Lemma HDisjointToDisjoint : forall (D D' : ctx), DestOnly D -> hvarsᴳ(D) ## hvarsᴳ(D') -> D # D'.
Proof.
...
Lemma DisjointToHDisjoint : forall (D D' : ctx), D # D' -> hvarsᴳ(D) ## hvarsᴳ(D').
Proof.
  intros * DisjointDDp.
  unfold HDisjoint. unfold NatSet.Empty. intros h HinInter.
...
Lemma EmptyIsHDisjointRight : forall (H : hvars), H ## NatSet.empty.
Proof.
  intros H. unfold HDisjoint. unfold NatSet.Empty. intros h Hin. rewrite NatSet.inter_spec in Hin. destruct Hin. inversion H1.
Qed.
Lemma EmptyIsHDisjointLeft : forall (H : hvars), NatSet.empty ## H.
Proof.
  intros H. unfold HDisjoint. unfold NatSet.Empty. intros h Hin. rewrite NatSet.inter_spec in Hin. destruct Hin. inversion H0.
Qed.

Lemma IsSubtype_refl : forall (m : mode), ModeSubtype m m.
Proof.
...
Lemma SingletonDestIsDestOnly : forall (h : hvar) (m n : mode) (T : type), DestOnly ᴳ{+ h : m ⌊ T ⌋ n}.
Proof.
...
Lemma SingletonVarIsVarOnly : forall (x : var) (m : mode) (T : type), VarOnly ᴳ{ x : m ‗ T}.
Proof.
...
Lemma SubsetDisjoint : forall (H1 H2 H3 : hvars), NatSet.Subset H1 H2 -> HDisjoint H2 H3 -> HDisjoint H1 H3.
Proof.
  intros * H1H2 H2H3. unfold HDisjoint, NatSet.Subset, NatSet.Empty in *.
...
Lemma DestOnlyWkNoVar : forall (D : ctx), DestOnly D -> NoVar D.
Proof.
...
Lemma DisposableOnlyWkVarOnly : forall (P : ctx), DisposableOnly P -> VarOnly P.
Proof.
...
Lemma SimplifyDisposableInTyC: forall (P D : ctx), DisposableOnly P -> DestOnly (P ᴳ+ D) -> (P ᴳ+ D) = D.
Proof.
  ...
  { apply ext_eq. intros n. destruct (In_dec n P) as [[y h_inP]|h_ninP].
    - unfold DisposableOnly in DisposP. specialize (DisposP n y h_inP). unfold IsDisposable in DisposP.
      unfold DestOnly in DestOnlyPuD. assert (In n P) as inP. { exists y. assumption. } assert (In n (P ᴳ+ D)) as InPuD. { apply InUnionLeft. assumption. } destruct InPuD as (y' & mapstoPuD). specialize (DestOnlyPuD n y' mapstoPuD). unfold IsDest in DestOnlyPuD. destruct n; contradiction.
    - apply nIn_iff_nMapsTo. assumption.
  }
  rewrite Pempty. symmetry. apply UnionIdentityLeft.
Qed.

Lemma VarOnlyNoVarUnionIsDisjoint : forall (P1 G2 : ctx), VarOnly P1 -> NoVar G2 -> P1 # G2.
Proof.
...
Lemma DestOnlyAtXIsNone : forall (D : ctx) (x : var), DestOnly D -> D (ˣ x) = None.
Proof.
...
Lemma VarOnlyAtHIsNone : forall (P : ctx) (h : hvar), VarOnly P -> P (ʰ h) = None.
Proof.
...
Lemma DestOnlyUnionSingletonVarAtX : forall (D : ctx) (x : var) (m : mode) (T : type), DestOnly D -> (D ᴳ+ ᴳ{ x : m ‗ T}) (ˣ x) = Some (ₓ m ‗ T).
Proof.
  intros * DestOnlyD.
...
Lemma IsSubtypeLinNuIsLinNu : forall (m : mode), ModeSubtype (¹ν) m <-> m = ¹ν.
Proof.
  intros m. split.
...
Ltac hauto_ctx :=
  hauto
    ...
    use:
        ...
        HmaxSubset,
        hvarsEmpty,
        EmptyIsHDisjointRight,
        EmptyIsHDisjointLeft,
        IsSubtype_refl,
        ...
        (* CompatibleVar_DestOnlyUnionSingleton, *)
        (IsLinProof (Fin 0)),
        (IsLinProof (Fin 1)),
        (IsFinAgeProof Lin 0),
        (IsFinAgeProof Lin 1),
...
Ltac term_Val_no_dispose D :=
  assert (DisposableOnly ᴳ{}) as DisposEmpty by (exact EmptyIsDisposableOnly); rewrite UnionIdentityLeft with (G := D); apply Ty_term_Val with (P := ᴳ{}); trivial.
...
Lemma TyR_val_DestOnlyValHole : forall (D : ctx) (h : hvar) (T : type), (D ⫦ ᵛ- h : T) -> DestOnly D -> False.
Proof.
...
Lemma Ty_ectxs_DestOnlyD : forall (D : ctx) (C : ectxs) (T U0 : type) (TyC: D ⊣ C : T ↣ U0), DestOnly D.
Proof.
  intros * TyC. induction TyC.
  all: crush.
Qed.

Lemma Ty_ectxs_HDisjoint : forall (C : ectxs) (D D' : ctx) (T U0 : type) (TyC : D ⊣ C : T ↣ U0), hvars©(C) ## hvarsᴳ(D') -> D # D'.
Proof.
  ...
  apply HDisjointToDisjoint in H. assumption. apply Ty_ectxs_DestOnlyD in TyC'. assumption.
Qed.

Lemma Ty_ectxs_LinFinOnlyD : forall (D : ctx) (C : ectxs) (T U0 : type) (TyC: D ⊣ C : T ↣ U0), LinOnly D /\ FinAgeOnly D.
Proof.
  ...
  - hauto lq: on use: LinOnlyUnionEquiv, LinOnlyStimesBackward, FinAgeOnlyUnionBackward, FinAgeOnlyStimesBackward, LinOnlyWkValidOnly.
  - assert (LinOnly (¹↑ ᴳ· D1)).
      { hauto use: LinOnlyUnionEquiv, LinOnlyStimesForward, (IsLinProof (Fin 1)). }
    assert (FinAgeOnly (¹↑ ᴳ· D1)).
      { hauto use: FinAgeOnlyUnionBackward, FinAgeOnlyStimesForward, (IsFinAgeProof Lin 1). }
    assert ((D1 ᴳ+ D2) # (ᴳ- D3)).
      { apply (Ty_ectxs_HDisjoint C (D1 ᴳ+ D2) (ᴳ- D3) (U ⧔ T') U0); tauto. }
    assert ((¹↑ ᴳ· D1) # D3).
...
Lemma LinOnlyWithStimes_dec : forall (D1 D2 : ctx) (m' : mode), IsValid m' -> LinOnly (m' ᴳ· D1 ᴳ+ D2) -> { IsLin m' } + { IsUr m' /\ D1 = ᴳ{} }.
Proof.
...
Proof.
  ...
  - rename x into Hu.
    ...
    assert (D1 = ᴳ{}). { destruct (LinOnlyWithStimes_dec D1 D2 m' Validmp LinOnlyD). inversion H0. inversion i. congruence. destruct a. assumption. }
...
Proof.
    ...
    - (* Sem-eterm-MapRedAOpenFoc *)
      ...
      assert ((D2 ᴳ+ D11) # (D13 ᴳ[ hvarsᴳ(ᴳ- D13) ⩲ (maxᴴ(hvars©(C)) + 1)])) as DisjointD2uD11D13. {
          {  apply DisjointNestedLeftEquiv. assert (NatSet.Subset hvarsᴳ(D11 ᴳ+ D12 ᴳ+ D2) hvars©(C)).
            ...
            { assert (NatSet.Subset hvarsᴳ(D2) hvars©(C)).
              ...
              { apply hvars_max_HDisjoint; rewrite Nat.add_comm; unfold lt, plus; apply le_n_S; tauto. }
              assert (hvarsᴳ(D2) ## hvarsᴳ( D13 ᴳ[ hvarsᴳ(ᴳ- D13) ⩲ (maxᴴ(hvars©(C)) + 1)])).
              { unfold HDisjoint. rewrite hvarsMinusEq. rewrite hvarsFullShiftEq. tauto. }
              apply HDisjointToDisjoint; crush.
            }
        ...
        { assert (¹↑ ᴳ· (D2 ᴳ+ D11) ᴳ+ D13 ᴳ[ hvarsᴳ(ᴳ- D13) ⩲ (maxᴴ(hvars©(C)) + 1)] = (¹ν ᴳ· (¹↑ ᴳ· D11 ᴳ+ D13) ᴳ[ hvarsᴳ(ᴳ- D13) ⩲ (maxᴴ(hvars©(C)) + 1)]) ᴳ+ ¹↑ ᴳ· D2) as ctxEq.
          ...
          assert (LinOnly ((¹ν ᴳ· (¹↑ ᴳ· D11 ᴳ+ D13) ᴳ[ hvarsᴳ(ᴳ- D13) ⩲ (maxᴴ(hvars©(C)) + 1)]) ᴳ+ ¹↑ ᴳ· D2)).
            { apply LinOnlyUnionEquiv; repeat split.
              { apply LinOnlyStimesForward. exact (IsLinProof (Fin 0)). apply LinOnlyHvarShiftEquiv. apply LinOnlyUnionEquiv; repeat split. apply LinOnlyStimesForward. exact (IsLinProof (Fin 1)). crush.
              assumption. crush. }
              { apply LinOnlyStimesForward. exact (IsLinProof (Fin 1)). crush. }
      ...
      assert (LinOnly (D11 ᴳ+ D12 ᴳ+ D2)) as LinOnlyD. { apply (Ty_ectxs_LinFinOnlyD (D11 ᴳ+ D12 ᴳ+ D2) C (U ⧔ T') U0). tauto. }
      assert (hvars©( C) ## hvarsᴳ( ᴳ- D13 ᴳ[ hvarsᴳ( ᴳ- D13) ⩲ (maxᴴ( hvars©( C)) + 1)])) as hvarsDisjoint.
        { rewrite <- MinusHvarShiftEq. rewrite hvarsFullShiftEq. apply hvars_max_HDisjoint with (h' := maxᴴ(hvars©(C)) + 1); rewrite Nat.add_comm; unfold lt, plus; apply le_n_S; reflexivity. }
        ...
        { apply LinOnlyUnionEquiv. rewrite <- UnionAssociative. rewrite UnionCommutative. tauto. }
        {
          {  apply DisjointNestedLeftEquiv. assert (NatSet.Subset hvarsᴳ(D11 ᴳ+ D12 ᴳ+ D2) hvars©(C)).
            ...
            { assert (NatSet.Subset hvarsᴳ(D12) hvars©(C)).
              ...
              { unfold HDisjoint. rewrite hvarsMinusEq. rewrite hvarsFullShiftEq. tauto. }
              apply HDisjointToDisjoint. crush. assumption.
    ...
    - (* Sem-eterm-AOpenUnfoc *)
      ...
      assert (D1 ᴳ+ D2 ⊢ ᵥ₎ hvarsᴳ(ᴳ- D3) ⟨ v2 ❟ v1 ⟩ : U ⧔ T) as TyA.
        { term_Val_no_dispose (D1 ᴳ+ D2). apply Ty_ectxs_HDisjoint with (D := D1 ᴳ+ D2) (D' := (ᴳ- D3)) (C := C) (T := U ⧔ T) (U0 := U0) in hvarsDisjoint. apply TyR_val_A. all: trivial. crush.
         }
      assert (LinOnly (D1 ᴳ+ D2)) as LinOnlyD.
        { apply (Ty_ectxs_LinFinOnlyD (D1 ᴳ+ D2) C (U ⧔ T) U0). tauto. }
...
Proof.
  ...
  - assert (P ᴳ+ ᴳ{ x : m ‗ T} = ᴳ{ x : m ‗ T}) as elimP. { apply SimplifyDisposableInTyC; tauto. } rewrite elimP in *.
    exfalso. unfold DestOnly in DestOnlyD. specialize (DestOnlyD (ˣ x) (ₓ m ‗ T)). unfold ctx_singleton in DestOnlyD. rewrite singleton_MapsKeyTo in DestOnlyD. specialize (DestOnlyD eq_refl). unfold IsDest in DestOnlyD. contradiction.
  ...
  - rename Tyt into TyMap, t0 into t, Tyt0 into Tyt, P1 into D1, P2 into D2. destruct (NotVal_dec t).
    * destruct e; subst. rename x0 into v. inversion Tyt; subst. inversion TyRv; subst. { exfalso. apply TyR_val_DestOnlyValHole with (D := ᴳ{- h : U ⧔ T ‗ ¹ν}) (h := h) (T := U ⧔ T); tauto. }
      rename D2 into D', D0 into D2. assert (DestOnly (P ᴳ+ (D1 ᴳ+ D2))) as DestOnlyPuD1uD2. { crush. } rewrite (SimplifyDisposableInTyC P (D1 ᴳ+ D2) DisposP DestOnlyPuD1uD2) in *.
  ...
  - rename Tyt into TyToA, Tyt0 into Tyt, t0 into t. destruct (NotVal_dec t).
    * destruct e; subst. rename x into v. inversion Tyt; subst. inversion TyRv; subst. { exfalso. apply TyR_val_DestOnlyValHole with (D := ᴳ{- h : T ⧔ ① ‗ ¹ν}) (h := h) (T := T ⧔ ①); tauto. }
      inversion TyRv1; subst. { assert (DestOnly (¹↑ ᴳ· D1 ᴳ+ D3)). { crush. } exfalso. apply TyR_val_DestOnlyValHole with (D := (¹↑ ᴳ· D1 ᴳ+ D3)) (h := h) (T := ①). all:tauto. }
      assert (¹↑ ᴳ· D1 ᴳ+ D3 = ᴳ{}) as Empty.
  ...
  - rename Tyt into TyFillC, Tyt0 into Tyt, t0 into t, P1 into D1, P2 into D2. destruct (NotVal_dec t).
    * destruct e; subst. rename x into v. destruct (NotVal_dec t').
      + destruct e; subst. rename x into v'. inversion Tyt; subst. inversion TyRv; subst. { exfalso. apply TyR_val_DestOnlyValHole with (D := ᴳ{- h : ⌊ U ⌋ n ‗ ¹ν}) (h := h) (T := ⌊ U ⌋ n); tauto. } rename H1 into DestOnlyD'. inversion Tytp; subst. rename TyRv0 into TyRvp. inversion TyRvp; subst. { exfalso. apply TyR_val_DestOnlyValHole with (D := ᴳ{- h0 : U ⧔ T ‗ ¹ν}) (h := h0) (T := U ⧔ T); tauto. } assert (DestOnly (P0 ᴳ+ (D1 ᴳ+ D2))) as DestOnlyP0uD1uD2. { crush. } rewrite (SimplifyDisposableInTyC P0 (D1 ᴳ+ D2) DisposP0 DestOnlyP0uD1uD2) in *.
      exists
