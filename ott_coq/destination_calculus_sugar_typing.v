...
Theorem Ty_term_termS_Alloc : forall (P : ctx) (T : type), DisposableOnly P -> P ⊢ alloc : T ⧔ (⌊ T ⌋ ¹ν).
Proof.
  intros P T DisposP.
  unfold termS_Alloc.
...
Theorem Ty_term_termS_FromA' : forall (P : ctx) (t : term) (T : type), P ⊢ t : T ⧔ ① -> P ⊢ from⧔' t : T.
Proof.
  intros * Tyt.
  unfold termS_FromA'.
...
Lemma Ty_term_termS_FillLeaf : forall (P1 P2 : ctx) (t t' : term) (T : type) (n : mode), IsValid n -> P1 ⊢ t : ⌊T⌋n -> P2 ⊢ t' : T -> P1 ᴳ+ (¹↑ · n) ᴳ·P2 ⊢ t ˢ⨞ t' : ①.
Proof.
  intros * Validn Tyt Tyt'.
  unfold termS_FillLeaf.
  ...
  apply Ty_term_FillC with (U := T); trivial.
  apply Ty_term_ToA. assumption.
Qed.

Lemma Ty_term_termS_F : forall (P2 : ctx) (x: var) (m: mode) (u : term) (T U : type), UserDefined P2 -> IsValid m -> P2 # ᴳ{ x : m ‗ T} -> P2 ᴳ+ ᴳ{ x : m ‗ T} ⊢ u : U -> P2 ⊢ ˢλ x ⁔ m ⟼ u : T ⁔ m → U.
Proof.
  intros * UserDefinedP2 Validm DisjointP2x Tyu.
  unfold termS_F.
  apply Ty_term_termS_FromA'.
  rewrite union_empty_l_eq with (G := P2).
  apply Ty_term_Map with (T := ⌊T ⁔ m → U⌋ ¹ν).
  { apply UserDefined_Disjoint; trivial. lia. }
  apply Ty_term_termS_Alloc. apply DisposableOnly_empty.
...
Lemma Ty_term_termS_L : forall (P2 : ctx) (t : term) (T1 T2 : type), UserDefined P2 -> P2 ⊢ t : T1 -> P2 ⊢ ˢInl t : T1 ⨁ T2.
Proof.
  ...
  apply Ty_term_termS_FillLeaf with (T := T1); trivial.
  constructor.
...
Lemma Ty_term_termS_R : forall (P2 : ctx) (t : term) (T1 T2 : type), UserDefined P2 -> P2 ⊢ t : T2 -> P2 ⊢ ˢInr t : T1 ⨁ T2.
Proof.
  intros * UserDefinedP2 Tyt.
  unfold termS_R.
  apply Ty_term_termS_FromA'.
  ...
  rewrite union_commutative.
  replace (¹↑) with (¹↑ · ¹ν). 2:{ cbn. reflexivity. }
  apply Ty_term_termS_FillLeaf with (T := T2); trivial.
  constructor.
  apply Ty_term_FillR with (T1 := T1) (T2 := T2); trivial.
  rewrite union_empty_l_eq with (G := ᴳ{ 0 : ¹ν ‗ ⌊ T1 ⨁ T2 ⌋ ¹ν}). apply Ty_term_Var. { apply DisposableOnly_empty. } { apply Disjoint_empty_l. } { repeat constructor. }
Qed.

Lemma Ty_term_termS_E : forall (P2 : ctx) (m : mode) (t : term) (T : type), UserDefined P2 -> IsValid m -> P2 ⊢ t : T -> m ᴳ· P2 ⊢ ˢᴇ m ⁔ t : ! m ⁔ T.
Proof.
  ...
  apply Ty_term_termS_FillLeaf with (P2 := P2) (T := T); trivial.
...
Lemma Ty_term_termS_P : forall (P21 P22 : ctx) (t1 t2 : term) (T1 T2 : type), UserDefined P21 -> UserDefined P22 -> P21 ⊢ t1 : T1 -> P22 ⊢ t2 : T2 -> (P21 ᴳ+ P22) ⊢ ˢ(t1, t2) : T1 ⨂ T2.
Proof.
  ...
  apply Ty_term_termS_FillLeaf with (P2 := P21) (T := T1); trivial.
  constructor.
  ...
  apply Ty_term_termS_FillLeaf with (P2 := P22) (T := T2); trivial.
  constructor.
...
Theorem Ty_termS_coherency : forall (P : ctx) (t : term) (T : type), P ˢ⊢ t : T -> P ⊢ t : T.
Proof.
  intros * Tyst. inversion Tyst; subst. inversion Tyst; subst.
  - apply Ty_term_termS_Alloc; trivial.
  - apply Ty_term_termS_FromA'; trivial.
  - replace (mode_times' ((¹↑ :: nil) ++ (n :: nil) ++ nil)) with (¹↑ · n). 2:{ cbn. destruct n; try destruct p; try destruct p2, a2; cbn; try destruct m, a; cbn; try rewrite Nat.add_0_r; trivial. } apply Ty_term_termS_FillLeaf with (T := T0); trivial.
  - apply Ty_term_termS_F; trivial.
  - apply Ty_term_termS_L; trivial.
  - apply Ty_term_termS_R; trivial.
  - apply Ty_term_termS_E; trivial.
  - apply Ty_term_termS_P; trivial.
Qed.
