...
%.diff.tex: compare-to/%.tex %.tex
	latexdiff --exclude-textcmd="section,subsection,paragraph,caption" --exclude-safecmd="improvement,critical,unsure,bgroup,SetPrefix,egroup,ottdefnTyXXval,CTyVal,CSep" --add-to-config "FLOATENV=ottfig[\w\d*@]*,FLOATENV=codefig[\w\d*@]*" --math-markup=whole $^ > $@
